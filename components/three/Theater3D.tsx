/* eslint-disable */
"use client";
// 3D 剧场 · 两分钟看懂 CEO 陪跑（网页实时渲染，替代直播闲时位）中英双语
...
const L = {
  zh: {
    live: "直播间", heading: "3D 剧场 · 两分钟看懂 CEO 陪跑",
    sub: "3D 卡通实时渲染 · 自动播放 · 可暂停 / 拖动进度",
    tagline: "真打法。真工具。真结果。", cta: "预约一次 CEO 陪跑沟通", ctaHref: "/contact",
    note: "※ 本段为网页实时 3D 渲染（非视频文件），加载快、可交互。8 个工具已内置同风格 3D 卡通「使用页面讲解」——在下方工具卡片点「▶ 3D 演示」播放。",
    caps: [
      "30 年一线经营者：GE 高管十年 · 六西格玛黑带大师 · 好孩子集团 CEO 十年",
      "CEO 陪跑——不是讲课，是和你一起下场打仗",
      "富耐克 2025：净利润 +140% · OEE +20%",
      "打法沉淀成 8 个 AI 数字工具，登录后全部免费使用",
      "真打法。真工具。真结果。",
    ],
    signClient: "陪跑企业", barOEE: "OEE +20%", barProfit: "净利润 +140%",
  },
  en: {
    live: "LIVE", heading: "3D Theater · CEO advisory in two minutes",
    sub: "Real-time 3D · autoplay · pause / scrub",
    tagline: "Real playbook. Real tools. Real results.", cta: "Book a CEO advisory conversation", ctaHref: "/en/contact",
    note: "※ Rendered live in your browser (not a video file) — fast and interactive. Each of the 8 tools has a same-style 3D walkthrough — click ▶ on any tool card below.",
    caps: [
      "30 years as a top operating leader: 10y GE GM · Six Sigma Master Black Belt · 10y CEO of Goodbaby",
      "CEO advisory — not lectures; we step into the fight with you",
      "Funik 2025: net profit +140% · OEE +20%",
      "The playbook, distilled into 8 AI tools — all free after login",
      "Real playbook. Real tools. Real results.",
    ],
    signClient: "Client", barOEE: "OEE +20%", barProfit: "Net profit +140%",
  },
};
const CAMS = [
...
export function Theater3D({ lang = "zh" }: { lang?: Lang }) {
  const tx = L[lang];
  ...
  useEffect(() => {
    ...
    const CAPS = tx.caps.map((text, i) => ({ t: [0, 8, 18, 30, 40][i], end: [8, 18, 30, 40, 46][i], text }));
    ...
    const factory = new THREE.Group();
    {
      ...
      const sign = label(tx.signClient, "#cfd8ea", 2.0); sign.position.set(0, 2.9, 0); factory.add(sign);
    ...
    const bars = new THREE.Group();
    {
      const defs = [
        ...
        { x: 1.0, h: 2.1, c: 0x38c8dd, text: tx.barOEE },
        { x: 2.0, h: 3.0, c: 0xd4a63f, text: tx.barProfit },
      ];
      ...
      defs.forEach((d) => {
        ...
        let lb: any = null;
        if (d.text) { lb = label(d.text, "#" + d.c.toString(16).padStart(6, "0"), 2.6); lb.position.set(d.x, d.h + 0.55, 0); bars.add(lb); }
    ...
    function camAt(t: number) {
      ...
      camera.lookAt(a.look[0] + (b.look[0] - a.look[0]) * k, a.look[1] + (b.look[1] - a.look[1]) * k, a.look[2] + (b.look[2] - a.look[2]) * k);
    }
    ...
    function update(t: number) {
      ...
      if (capRef.current) { capRef.current.textContent = c ? c.text : ""; (capRef.current.parentElement as HTMLElement).style.opacity = c ? "1" : "0"; }
      ...
      if (t < 8) {
        ...
        ch.g.position.set(x, Math.abs(Math.sin(t * 7)) * 0.05, 0.6); ch.g.rotation.y = Math.PI / 2; walk = 1;
      } else if (t < 30) {
        ch.g.position.set(2.8, 0, 0.6); ch.g.rotation.y = Math.PI / 2.6;
        ch.armR.rotation.x = -1.9 + Math.sin(t * 2.6) * 0.07; ch.armR.rotation.z = -0.3;
        ch.armL.rotation.x = Math.sin(t * 1.4) * 0.08;
      } else if (t < 40) {
        ch.g.position.set(0, 0, 0); ch.g.rotation.y = (t - 30) * 0.5;
        ch.armL.rotation.x = -0.5; ch.armR.rotation.x = -0.5; ch.armL.rotation.z = 0.5; ch.armR.rotation.z = -0.5;
      } else {
        ch.g.position.set(0, 0, 0); ch.g.rotation.y = Math.sin(t) * 0.15;
        ch.armR.rotation.x = -2.35; ch.armR.rotation.z = Math.sin(t * 6) * 0.35; ch.armL.rotation.x = 0; ch.armL.rotation.z = 0;
      }
      if (walk) {
        const w = Math.sin(t * 7) * 0.68; ch.legL.rotation.x = w; ch.legR.rotation.x = -w; ch.armL.rotation.x = -w * 0.62; ch.armR.rotation.x = w * 0.62;
      } else if (t < 8 || (t >= 30 && t < 40)) { ch.legL.rotation.x = 0; ch.legR.rotation.x = 0; }
      ...
      (bars as any).userData.items.forEach((it: any, i: number) => {
        ...
        it.mesh.scale.y = Math.max(0.001, k * it.h); it.mesh.position.y = it.mesh.scale.y / 2;
      ...
      (tools as any).userData.cubes.forEach((cb: any, i: number) => {
        const a = (i / 8) * Math.PI * 2 + t * 0.7; const tilt = i % 2 ? 0.35 : -0.3;
    ...
    function loop() {
      ...
      last = now; update(T); renderer.render(scene, camera);
    }
    ...
    eng.current = {
      ...
      progSeek: (e: React.MouseEvent) => { const r = progRef.current!.getBoundingClientRect(); T = Math.max(0, Math.min(DUR, ((e.clientX - r.left) / r.width) * DUR)); playT(true); },
    };
  ...
  return (
    <section className="text-white" id="theater" style={{ background: "#0b0d12" }}>
      <style>{`
        ...
        .t3d-cap span{background:rgba(8,10,14,.72);border:1px solid rgba(255,255,255,.14);color:#f3e9d2;padding:10px 22px;border-radius:8px;font-size:16.5px;letter-spacing:.3px;backdrop-filter:blur(4px);text-align:center}
      ...
      <div className="mx-auto max-w-5xl px-5 py-14 sm:px-8">
        <div className="mb-2 flex flex-wrap items-baseline gap-4">
          <span className="t3d-live"><i />{tx.live}</span>
          <h2 className="text-2xl font-light sm:text-3xl">{tx.heading}</h2>
        </div>
        <p className="mb-5 text-sm text-neutral-400">{tx.sub}</p>
        <div className="t3d-wrap">
          ...
          <div className="t3d-end" ref={endRef}>
            ...
            <p className="text-neutral-200">{tx.tagline}</p>
            <a href={tx.ctaHref} className="mt-2 rounded bg-[var(--gold)] px-6 py-2.5 text-sm font-semibold text-white hover:opacity-90">{tx.cta}</a>
          </div>
        ...
        <p className="mt-4 text-xs leading-6 text-neutral-500">{tx.note}</p>
      </div>
    </section>
  );
}
